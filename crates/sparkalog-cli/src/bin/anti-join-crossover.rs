use sparkalog_execution::{
    CudaStream, anti_join_cpu_parallel, anti_join_cpu_serial, anti_join_cuda,
    distinct_cpu_parallel, distinct_cuda, join_cpu_parallel, join_cuda,
};
...
struct Inputs {
    ...
    left: DistinctWorkspace,
    right: DistinctWorkspace,
}
...
impl Inputs {
    fn new() -> Result<Self, Box<dyn std::error::Error>> {
        Ok(Self {
            ...
            left: DistinctWorkspace::new()?,
            right: DistinctWorkspace::new()?,
        })
    }
}
...
fn prepare_inputs(
    ...
    match producer {
        Producer::Cpu => {
            ...
            distinct_cpu_parallel(edge, distinct_plan(), &mut inputs.right)?;
        }
        Producer::Gpu => {
            join_cuda(
                delta,
            ...
            .wait()?;
            distinct_cuda(edge, distinct_plan(), &mut inputs.right, stream)?.wait()?;
        }
    }
    Ok(())
}
...
fn run_anti_join(
    ...
    match backend {
        Backend::CpuSerial => anti_join_cpu_serial(left, right, anti_join_plan(), workspace)?,
        Backend::CpuParallel => anti_join_cpu_parallel(left, right, anti_join_plan(), workspace)?,
        Backend::Gpu => anti_join_cuda(left, right, anti_join_plan(), workspace, stream)?.wait()?,
    }
    Ok(())
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let config = parse_config().map_err(|message| format!("anti-join-crossover: {message}"))?;
    ...
    for delta_rows in delta_sizes {
        ...
        let left_rows = inputs.left.output().len();
        let right_rows = inputs.right.output().len();
        let mut reference = AntiJoinWorkspace::new()?;
        anti_join_cpu_serial(
            inputs.left.output().view(),
            inputs.right.output().view(),
            anti_join_plan(),
            &mut reference,
        )?;
        ...
        for producer in producers {
            for backend in backends {
                let mut workspace = AntiJoinWorkspace::new()?;
